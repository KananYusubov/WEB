...
function GetDetailValueFromDetailType(value, type = detailTypes.location) {
    switch (type) {
        ...
        case detailTypes.url:
            return `<a href="${value.startsWith('https://') ? value : 'https://' + value}" target="_blank">${value}</a>`;
...
function ReadAdditionalInfo(response) {
    AddInfoCard('created_at', 'Profile Creation Date', GetDate(response.created_at));
    ...
    ReadUrlDataCount(`https://api.github.com/users/${response.login}/starred`, 'star', 'Starred Repositories');
}
